...
## The country pages are finished

All 31 now carry written five-lens analysis. Poland's is hand-written; the other 30 are
generated by `narrate.py` from the verified series — 135 lens analyses in all. Generated rather
than written by hand because 27 hand-written analyses would drift the moment a series was
refreshed, and each one would be a chance to say something the data does not support. The
generator never asserts causation the study has not established: `sm.js` checks every analysis
for length, for the word "pending", and for overclaiming phrases.

**Build order matters.** `gen_narrative.py` rewrites the layout files with placeholder prose, so
it must run *before* `narrate.py`:
